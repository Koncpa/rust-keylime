...
COVERAGE_URL=$( grep "rust_coverage.txt report is available at" ${TMPFILE} | egrep -o "${WEBDRIVE_URL}.*\.txt" )
echo "COVERAGE_URL=${COVERAGE_URL}"
if [ -z "${COVERAGE_URL}" ]; then
    echo "Could not parse rust_coverage.txt URL at ${WEBDRIVE_URL} from test log ${TF_TESTLOG}"
    exit 5
fi
